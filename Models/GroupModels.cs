...
namespace tourismAPi.Models
{
    public class GroupClass
    {
        public sitemsModels GetSearchModels(string licenses, string cuurip)
        {
            ...
            mainRows = new database().checkSelectSql("mssql", "sysstring", "select userid,username,idcard,birthday,place,category,traffic,location from web.tourism where groupid = @groupid order by id asc;", dbparamlist);
            ...
            {
                items.Add(new Dictionary<string, object>() { { "userid", dr["userid"].ToString().TrimEnd() }, { "username", dr["username"].ToString().TrimEnd() }, { "idcard", dr["idcard"].ToString().TrimEnd() }, { "birthday", dr["birthday"].ToString().TrimEnd() }, { "showPlace", dr["place"].ToString().TrimEnd() == "1" } });
            }
